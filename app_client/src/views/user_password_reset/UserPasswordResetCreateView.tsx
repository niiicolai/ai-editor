...
import { useCreateUserPasswordReset } from "../../hooks/useUserPasswordReset";
import { useState } from "react";
import { Link } from "react-router-dom";

function UserPasswordResetCreateView() {
  const { mutateAsync, isPending, error } = useCreateUserPasswordReset();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await mutateAsync({ email: formData.get("email") as string });
      setIsSent(true);
    } catch (err) {
        ...
        {!isSent && (
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            <div className="rounded-xl shadow-sm bg-gray-50/80 p-6 space-y-4">
              <div data-testid="email-user-password-reset-input">
                <label
                  htmlFor="email"
            ...
            {(error || formError) && (
              <div
                ...
                data-testid="user-password-reset-create-error"
              >
            ...
            <div>
              <button
                data-testid="user-password-reset-create-button"
                type="submit"
                disabled={isPending}
...
export default UserPasswordResetCreateView;
