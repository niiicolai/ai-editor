...
function ChatSessionsComponent(props: ChatShowComponentProps) {
    const { setSelectedSession, setShowSessions, selectedSession } = props;
    const limit = 7;
    ...
    return (
        <div className="w-96 h-screen main-bgg text-white bottom-0 border-r border-gray-800 bg-gray-50 flex flex-col">
            {/* Sessions Header */}
            <div className="px-4 py-3 border-b border-gray-800">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-medium text-gray-100">Chat Sessions</h2>
                    <button
                ...
                <p className="text-sm text-gray-200">Select a session to view messages</p>
            </div>
                ...
                {sessions?.map((session) => (
                    <div className="flex" key={session._id}>
                        <button
                            ...
                            className={`flex-1 overflow-hidden truncate px-4 py-3 text-left flex items-center space-x-3 cursor-pointer ${selectedSession?._id === session._id
            ...
            {/* Sessions Pagination */}
            {data && data.pages > 1 && (
                <div className="px-4 py-3 border-t border-gray-800 main-bgg">
                    <div className="flex items-center justify-between">
                        <button
                            onClick={prevPage}
                            disabled={page === 1}
                            className="cursor-pointer inline-flex items-center px-2 py-1 text-xs font-medium rounded-md text-gray-700 bg-white border border-gray-300 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            Previous
                        </button>
